...
export async function PUT(
  ...
  try {
    ...
    const existing = await db.select({ userId: jurnalMengajar.userId }).from(jurnalMengajar).where(eq(jurnalMengajar.id, id)).get();
    if (!existing || (scope && existing.userId !== scope)) {
      return apiError("Jurnal tidak ditemukan", 404);
    }
