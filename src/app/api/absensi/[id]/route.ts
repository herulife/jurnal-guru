...
export async function PUT(
  ...
  try {
    ...
    await db.update(absensi).set(updateData).where(eq(absensi.id, id));
    await addLog(session.id, "UPDATE_ABSENSI", `Update absensi ${id}`);
    return apiOk(null, "Absensi diperbarui");
  } catch (e: unknown) {
