...
> **Pembaruan deployment (18 Agustus 2026):** Aplikasi TIDAK lagi berjalan di Vercel — runtime sekarang VPS (`pm2 jurnal-guru`, `next start` port 3000), DB = file lokal `data.db`, akses publik via Cloudflare Tunnel (`guru-tunnel`) untuk `guru.cintabuku.site`. `src/db/index.ts` memakai fallback file lokal; binding D1 dinonaktifkan di VPS via env `D1_ACTIVE=false` (`.env.local`). Rollback: `/tmp/opencode/rollback-vps.env`.
...
| Database | SQLite | Adaptor ganda: **D1 Cloudflare** (via `getCloudflareContext`; dinonaktifkan di VPS via `D1_ACTIVE=false`) → fallback **Turso/libsql** (`DATABASE_URL`; tidak dipakai) → **file lokal `file:./data.db`** (AKTIF di VPS) (`src/db/index.ts`) |
...
| Build/Deploy | VPS (pm2 `jurnal-guru`, `next start` :3000, 18 Agustus 2026) | DNS `guru.cintabuku.site` → Cloudflare Tunnel `guru-tunnel` → localhost:3000; legacy Vercel (CNAME `cname.vercel-dns.com`) & Cloudflare worker `opennextjs-cloudflare` hanya utk rollback/domain lama |
...
Sumber: `src/db/schema.ts` (Drizzle ORM, SQLite). Semua id bertipe `text` (UUID v4). 18 tabel:
...
| VPS + pm2 (AKTIF, 18 Agu 2026) | `next start` port 3000 | runtime produksi `guru.cintabuku.site` via Cloudflare Tunnel | pm2: `jurnal-guru`, `cloudflared` |
| Vercel (rollback) | GitHub integration | auto-deploy dari push `main` (DNS tidak menunjuk lagi) | — |
| Cloudflare (legacy) | `opennextjs-cloudflare`, D1 | worker domain lama + opsi D1 (D1 dinonaktifkan di VPS via `D1_ACTIVE=false`) | `wrangler.*`, `db/index.ts` |
| Database file lokal (AKTIF) | `@libsql/client` `file:./data.db` | DB produksi di VPS (turunan data Turso) | `db/index.ts` |
