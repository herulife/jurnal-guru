...
## 17 Agu — Audit aplikasi lengkap + sistem dokumentasi admin (90aee5e, 8ba2de4, eb23e2c)
- Audit menyeluruh (20 seksi, 784+ baris, Bahasa Indonesia): Executive Summary, Technology Stack, Application Structure, Route Map, Navigation Map, Database Schema, ERD, User Flow, Feature Documentation, Dashboard Documentation, Authentication, API Documentation, UI/UX, Business Logic, Integration, Reusable Components, Technical Debt, Marketing Transformation, AI Marketing Assistant, Recommendations. File: `.agents/audits/2026-08-17-audit-ba6e63d.md` (format `YYYY-MM-DD-audit-<sha>.md`).
- Halaman admin baru `/documentation` (DocumentationView.tsx, pola AdminGuard + fs read seperti marketing-plan): per-section Copy, Copy All, Export Markdown (download .md), search per seksi, tab Audit History, tampil "Audit terakhir + commit". Menu sidebar Panel Admin → "Application Audit" (adminOnly).
- TEMUAN PENTING: 18 tabel Drizzle (bukan 16); auth JWT HS256 cookie 30d; scoping user/admin; plan gating Pro/Premium; middleware aktif (fix commit 136bc40); dark mode body.dark vs Tailwind dark: (media OS) masih campuran; rate limit in-memory; data_surat global tanpa scope user; tidak ada AI provider/analytics/WA API.
- CATATAN DEPLOY: commit 90aee5e sempat GAGAL build Vercel karena sisa marker merge conflict `<<<<<<< HEAD` di Sidebar.tsx lolos dari edit konflik (tsc lokal tidak mendeteksi). Fix commit 8ba2de4. Pelajaran: setelah resolve konflik rebase, grep `<<<<<<<|>>>>>>>` sebelum commit.
- VPS sudah sync eb23e2c; live terverifikasi /documentation 200 (konten di flight payload — pola AdminGuard render spinner dulu, sama seperti /marketing-plan).

## 20 Agu 2026 — Revisi harga live (29rb/49rb) + proyek demo teacher-benuatech
- **Harga final (live):** Gratis = Rp0; Pro = **Rp 29.000/6 bulan**; Premium = **Rp 49.000/6 bulan** (sebelumnya 49/99 di KODE, docs sudah 29/49 sejak 16 Agu — sekarang kode menyusul).
- File diubah: `src/lib/payment-plans.ts` (pro_6m=29000, premium_6m=49000; durasi lain disamakan: 1m=29rb, 3m=79rb, 12m=149rb, 24m=249rb), `src/app/checkout/page.tsx` (perBulan=4833), `src/app/page.tsx`, `(app)/subscription`, `(app)/faq`, `PlanGuard.tsx`, `UpgradeBanner.tsx`, `UserMenu.tsx` (29rb), `AuditFullDocumentation.tsx`.
- `src/app/api/social-proof/route.ts`: sekarang join `subscriptions` utk ambil `planId` (fallback amount: >=99000 atau ==49000 → Premium, else Pro). CATATAN: tabel `payments` TIDAK punya kolom plan_id; plan ada di `subscriptions.plan_id`.
- Deploy: `npx next build` (TS error planId → fix join) → `pm2 restart jurnal-guru --update-env`. Verifikasi live: landing tampil Rp 29.000/49.000, chunk JS berisi price:29/price:49.
- **Proyek demo baru:** folder `/home/ubuntu24/teacher-benuatech` (salinan proyek, BUKAN git repo) jalan via pm2 `jurnal-guru-2` (port 3001) + nginx `demo.benuatech.web.id` (A record existing 103.107.206.10, origin cert, proxy 127.0.0.1:3001). `guru.cintabuku.site` & tunnel TIDAK disentuh. Folder duplikat `teacher-dashboard-next` lama di dalamnya dipindah ke `/home/ubuntu24/backup-teacher-dashboard-next`.
- **GitHub baru:** repo `herulife/teacher-benuatech` (public, push manual pakai token ghp di `~/.cloudflared/github-token`; token gh CLI fine-grained TIDAK bisa create repo/push — pakai helper `-c credential.https://github.com.helper='!f(){ echo "username=x-access-token"; echo "password=$TOKEN"; }; f'`).
- **Kredensial VPS** (`~/.cloudflared/`, chmod 600): `api-token` (CF token cfut_… utk DNS zone benuatech.web.id — TIDAK punya akses tunnel config), `api-account` (candraloka81@gmail.com), `github-token` (ghp_… scope repo penuh), `sudo-pass` (password sudo ubuntu24). JANGAN commit ke repo.
- Catatan: token CF tidak bisa ubah konfigurasi tunnel (Not authorized) — jalur nginx dipilih utk demo.benuatech.web.id karena lebih cepat & tidak menyentuh tunnel.
