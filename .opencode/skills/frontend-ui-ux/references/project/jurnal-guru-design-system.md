# Design System Jurnal Guru (project-specific)

Produce production-ready, high-craft UI consistent with the existing Jurnal Guru teacher-dashboard SaaS. Reuse the tokens below — do not reinvent.
...
- **Reused CSS classes:** `.card`, `.btn` (`.btn-primary .btn-accent .btn-outline .btn-danger .btn-sm`), `.input`, `.label`, `.table-wrap`, `.modal-overlay`, `.modal-content`, `.fade-in`, `font-[Outfit]`, `font-[DM_Sans]` (if present), `min-h-[70px]`.
- **Layout:** `src/app/(app)/layout.tsx` with `<Sidebar/>`; pages are `"use client"`, container `.p-6 fade-in`, sticky `.header`.
