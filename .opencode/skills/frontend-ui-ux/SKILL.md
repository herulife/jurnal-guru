...
## Project tokens (Jurnal Guru)

This skill is installed inside the Jurnal Guru project. Before any UI work, read `references/project/jurnal-guru-design-system.md` — the project's actual design tokens (fonts, palette, reused `.card/.btn/.input/.modal-*` classes) take precedence over generic references. Do not reinvent tokens that already exist in the project.
